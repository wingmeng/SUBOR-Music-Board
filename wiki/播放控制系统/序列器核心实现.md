...
#### BPM 实时调整机制（2026-08-30 改）

Sequencer 支持播放过程中的实时 BPM 调整，采用「立即丢弃 + 防抖重启」：
...
Sequencer->>Engine : stopAll() 立即丢弃所有声
Sequencer->>Sequencer : 120ms 防抖（连点只重置定时器）
Sequencer->>Sequencer : restartFromCurrentPosition()
Sequencer->>Sequencer : 计算当前列并 scheduleAllNotes(currentCol)
Sequencer->>Sequencer : 重置 playbackWallStart（与音频基准对齐约 100ms）
Sequencer->>Timer : 定时器继续推进指示器
Sequencer-->>User : 调整完成
```
...
> 注（2026-08-30）：播放中实时变速/变调已改用 `stopAll()` 丢弃重排方案，`stopFrom` 当前无调用方，仅作为 MusicEngine 的保留工具存在。
...
**症状**：调整 BPM 后播放异常（节奏错乱、混响叠加）

**可能原因**：
1. 播放中变更未走 `requestPlaybackRestart()`（stopAll + 防抖重启）
2. 时间/基准计算错误（playbackWallStart 与调度基准未对齐）
3. 防抖定时器未在暂停/停止时清除

**解决步骤**：
1. 确认 setBpm/setKeySignature 命中 playing 时调用 requestPlaybackRestart
2. 检查 restartFromCurrentPosition 后 playbackWallStart 与 scheduleAllNotes 基准对齐（约 100ms）
3. 确认 pause()/stop() 清除了 restartTimer
