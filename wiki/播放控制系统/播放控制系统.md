...
- 时间调度机制
  - 列间隔：每列=八分音符，间隔=30/BPM秒
  - 音符时长：方波=列间隔，三角波略短（约0.93倍列间隔）
  - UI定时器以约50ms轮询，仅在列号变化时触发回调
- 实时BPM与调号切换
  - BPM变化：计算当前列，仅停止未来音符，当前列自然结束，随后从下一列按新BPM重新调度
  - 调号变化：计算当前列，仅停止未来音符，随后从下一列按新调号重新调度
  - 通过lastScheduleBaseTime与lastScheduleStartCol记录调度基线，配合stopFrom实现选择性停止
- 循环播放
  - 支持开启/关闭循环
...
- 节拍同步
  ...
  - Sequencer内部计算当前列，仅停止未来音符并从下一列重新调度，保持当前音符自然结束
...
UP->>SQ : setBpm(newBpm)
SQ->>ME : stopFrom(cutoffTime)
SQ->>SQ : scheduleAllNotes(nextCol)
...
SetKey --> Resched{"播放中？"}
Resched --> |是| StopFuture["stopFrom(cutoffTime)"]
StopFuture --> ReSchedule["从下一列重新调度"]
Resched --> |否| End(["结束"])
ReSchedule --> End
```
...
- 选择性停止
  - 仅停止未来音符，当前音符自然结束，避免卡顿
  - 使用activeOscillators映射追踪振荡器起始时间，实现高效筛选
- UI定时器优化
...
- BPM切换卡顿
  - 确保setBpm调用路径中仅停止未来音符，当前列自然结束
  - 检查_rescheduling防重入逻辑
- 调号切换异常
  - 确认setKeySignature触发了stopFrom与重新调度
  - 检查columnToMidi与noteToMidi映射是否正确
- 循环播放跳过
...
## 结论
播放控制系统通过“预调度 + 选择性停止 + UI定时器”的组合，在保证音准与时序精度的同时，实现了流畅的播放体验。Sequencer承担调度与状态管理，MusicEngine封装底层音频，usePlayback与UI组件提供直观的交互。BPM与调号的动态切换通过最小化重调度范围，确保播放过程平滑无卡顿。循环播放采用无缝重置策略，提升用户体验。整体架构清晰、职责分离，具备良好的可维护性与扩展性。
