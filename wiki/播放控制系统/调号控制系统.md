...
alt 播放中且非重调度
Seq->>Seq : 计算当前列位置
Seq->>Engine : stopFrom(cutoffTime)
Engine->>Engine : 停止未来音符
...
1. **计算截止时间**：基于当前播放位置计算未来音符的截止时间
2. **选择性停止**：仅停止当前列之后的音符，保留正在播放的音符
3. **重新调度**：从下一列开始使用新调号重新调度所有剩余音符
4. **状态同步**：保持播放进度和UI状态的一致性
...
**解决方法**：
1. 检查`lastScheduleBaseTime`的设置
2. 验证`playbackWallStart`的同步机制
3. 确认时间计算的精度
