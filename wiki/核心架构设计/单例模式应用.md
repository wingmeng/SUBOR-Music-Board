...
-timerId
-playbackWallStart
-lastScheduleBaseTime
-lastScheduleStartCol
-_rescheduling
+setScore(...)
+setKeySignature(...)
+setBpm(...)
...
- 实现要点
  ...
  - 提供 BPM 与调号变更时的选择性停止与重调度，避免卡顿与错拍。
- 生命周期控制
  - play/pause/stop：统一管理播放状态与定时器，暂停时记录当前位置，停止时清理定时器与音符。
  - scheduleAllNotes：一次性预调度所有剩余音符，匹配 jinglebell 的预调度模式。
- 音频冲突避免
  - 通过 getMusicEngine().stopFrom(...) 仅停止未来音符，当前音符自然结束，避免播放中断感。
  - 通过 lastScheduleBaseTime 与 lastScheduleStartCol 记录调度基线，确保 BPM/调号变更时的精确截断点。
...
- 选择性停止
  - BPM/调号变更时仅停止未来音符，避免当前音符被强制中断，减少卡顿与点击声。
- 资源复用
...
- 播放卡顿或错拍
  - BPM/调号变更时应使用选择性停止与重调度，避免直接停止所有音符。
- 资源泄漏
...
## 结论
MusicEngine 与 Sequencer 采用单例模式，是本项目在音频处理领域的关键设计决策。它确保了音频资源的唯一性与调度的一致性，避免了多实例引发的冲突与资源浪费；同时通过预调度与选择性停止策略，兼顾了播放精度与用户体验。尽管单例模式在测试与全局状态管理方面带来一定挑战，但通过清晰的生命周期控制与合理的接口设计，项目在功能完整性与可维护性之间取得了良好平衡。
...
- 选择性停止：在动态调整 BPM/调号时，优先使用 stopFrom 与重调度，避免卡顿。
