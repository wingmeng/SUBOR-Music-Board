...
- 职责
  ...
  - 实时 BPM 调整：仅停止未来音符，保留当前列正在发声的音符
  - 实时调号切换：仅停止未来音符，从下一列开始用新调号重新调度
  - 循环播放：到达末尾时重置起点并重新调度
  - UI 定时器：以固定周期轮询当前列号，触发 onPlay 回调
- 关键算法
  - 预调度：以 AudioContext 当前时间为基准，计算每列的绝对起始时间，批量调度三个声部
  - BPM 调整：计算当前列号与截止时间，调用 MusicEngine.stopFrom 停止未来音符，随后从下一列重新调度
...
- 事件与回调
  ...
  - onComplete：播放结束时触发
- 线程与并发
  - 通过 _rescheduling 标志防止快速多次调用 setBpm/setKeySignature 导致的竞争条件
...
- 选择性停止
  - 调整 BPM/调号时，仅停止未来音符，保留当前列正在发声的音符，避免卡顿
- 资源管理
  - stopAll/stopFrom 会清理活跃振荡器，防止内存泄漏与后台噪音
- 建议
  - 避免频繁调用 setBpm/setKeySignature，内部已有防抖标志
...
- BPM 调整卡顿
  - 确保 setBpm 调用发生在 playing 状态且非 _rescheduling 中
  - 避免在短时间内多次调用 setBpm
- 调号切换异常
  - 确保 setKeySignature 调用发生在 playing 状态且非 _rescheduling 中
...
## 结论
该播放控制系统通过“预调度 + UI 轮询”的架构实现了高精度、低延迟的节拍驱动播放。Sequencer 负责播放调度与状态管理，MusicEngine 提供底层音频渲染与选择性停止能力，usePlayback 将播放控制与状态暴露给 UI 层。系统在 BPM 实时调整、调号切换、循环播放等方面具备良好的用户体验与性能表现。
...
```mermaid
flowchart TD
Start(["setBpm 新 BPM"]) --> CheckPlaying{"播放中且非重调度？"}
CheckPlaying --> |否| End["返回"]
CheckPlaying --> |是| Mark["_rescheduling = true"]
Mark --> CalcCur["计算当前列与截止时间"]
CalcCur --> StopFuture["stopFrom(截止时间)"]
StopFuture --> RecalcStart["计算新起始时间"]
RecalcStart --> Resched["从下一列重新 scheduleAllNotes"]
Resched --> Unmark["_rescheduling = false"]
Unmark --> End
```
