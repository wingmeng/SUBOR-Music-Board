...
- 实时调整（2026-08-30 改）：setBpm 与 setKeySignature 在播放中动态调整时，调用 `requestPlaybackRestart()` 立即 stopAll 丢弃所有声，120ms 防抖后从当前指示器列整体重排，避免新旧音符叠加混响；同时更新 playbackWallStart 与调度基准对齐（约 100ms）保持 UI 同步。
...
- 乐谱结构：Column 为三元组，Score 为列数组；初始 125 列，随输入/导入动态扩展（不设硬上限）。
...
- 实时调整：BPM/调号变更时立即 stopAll 丢弃所有声，120ms 防抖后从当前指示器列整体重排，避免多版本叠加混响并保持音画同步。
...
- BPM/调号变更错乱（混响/错位）
  - 确认 setBpm/setKeySignature 命中 playing 时走 requestPlaybackRestart（stopAll + 防抖重启）；不要在播放中直接重建引擎。
