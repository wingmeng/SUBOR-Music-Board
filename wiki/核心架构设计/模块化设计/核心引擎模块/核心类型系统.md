...
- Score（乐谱）
  ...
  - 设计要点：固定列数（默认 125 列），实际有效长度由最后一列非空格音符决定
...
- 实时重调度
  - BPM/调号变更时，仅停止未来音符，当前列自然结束，减少卡顿
- 有效长度优化
...
- BPM 变更卡顿
  - 现象：播放中调整 BPM 时出现停顿
  - 原因：未使用 stopFrom 选择性停止未来音符
  - 处理：确保 sequencer.setBpm 使用重调度策略
  - 参考：[sequencer.ts:84-115](file://src/core/sequencer.ts#L84-L115)
