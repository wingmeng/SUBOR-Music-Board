...
- 音乐引擎 MusicEngine：封装 Web Audio API，提供单例访问、初始化、音符调度、停止控制与资源释放。核心特性包括：
  ...
  - 选择性停止：stopFrom 仅终止未来音符，避免播放中断。
- 序列器 Sequencer：负责播放状态、BPM/调号变更、UI 定时器与预调度逻辑。
  - 预调度：一次性计算所有音符的绝对时间并调度。
  - 实时调整：setBpm/setKeySignature 支持播放中切换，仅停止未来音符并从下一列重新调度。
...
- 预调度策略：计算 noteInterval 与 noteDuration，记录 lastScheduleBaseTime 与 lastScheduleStartCol，以便 setBpm 时进行选择性停止。
- 实时调整：setBpm 在播放中仅停止未来音符并从下一列重新调度，同时更新 playbackWallStart 以保持 UI 定时器同步。
...
- 选择性停止：setBpm/setKeySignature 仅停止未来音符，保留当前正在播放的音符，减少卡顿。
...
- BPM 切换异常：检查 lastScheduleBaseTime 与 lastScheduleStartCol 是否正确更新；确保 stopFrom 的截止时间计算正确。
- 三角波点击声：确认三角波增益包络的淡出时间与起止时间设置合理。
- 调号切换不生效：确认 setKeySignature 的 rescheduling 状态未被竞态覆盖；确保从下一列重新调度。
...
- 音符时长：方波填满整个八分音符间隔，三角波略短以匹配演示页面比例。
...
- noteInterval：每列八分音符（半拍），BPM 决定间隔。
