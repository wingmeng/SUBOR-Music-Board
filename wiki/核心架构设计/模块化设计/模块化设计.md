...
- 序列器（Sequencer）
  - 职责：根据乐谱与配置进行播放调度；管理播放状态、循环、UI 定时器；在播放中动态调整 BPM 与调号时，stopAll 立即丢弃所有声，120ms 防抖后从当前指示器列整体重排（2026-08-30 改）
...
- 实时变速/变调的丢弃重排：BPM/调号变更时 stopAll 丢弃所有声，120ms 防抖后从当前指示器列整体重排，杜绝叠加混响并保持音画同步
