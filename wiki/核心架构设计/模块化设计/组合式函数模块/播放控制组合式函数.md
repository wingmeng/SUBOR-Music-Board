...
### Sequencer 播放器
- 状态与属性：state、pausedColumn、loop、timerId、playbackWallStart、lastScheduleBaseTime、lastScheduleStartCol、_rescheduling
- 关键方法：
...
- 实时调整策略：
  - BPM 变化：只停止未来音符，当前列自然结束，避免卡顿；重新计算 playbackWallStart 并从下一列开始调度
  - 调号变化：同样只停止未来音符，从下一列开始用新调号重新调度
...
- 选择性停止：BPM/调号变更时仅停止未来音符，当前列自然结束，避免卡顿与点击声
...
- BPM 变更卡顿
  - 确认未在 _rescheduling 中重复调用 setBpm
  - 避免短时间内多次调用，等待当前 rescheduling 结束
- 调号切换异常
  - 确认当前列已正确计算并从下一列开始重新调度
  - 检查 stopFrom 是否正确传入 cutoffTime
- UI 不更新
...
## 结论
usePlayback 将播放状态、BPM/调号联动与 Sequencer 的事件回调无缝整合，配合 ControlBar/PlaybackControl 等 UI 组件，实现了简洁而强大的播放控制体验。Sequencer 的预调度与选择性停止策略在保证音质的同时兼顾性能，note-map 与 MusicEngine 则提供了准确的音符映射与稳定的音频输出。通过合理的响应式设计与事件传播，该方案易于扩展与维护。
