...
- 关键 API
  ...
  - moveCursor(col, voice)：移动光标，col 边界检查，voice 限制在 0..2
  - resetScore()：清空乐谱并重置光标
  - loadScore(newScore)：加载新乐谱，最多 DEFAULT_COLUMNS 列，不足则用空白列补齐
...
- 乐谱装载与重置
  - loadScore：截断至 DEFAULT_COLUMNS，不足则补齐空白列，重置光标
  - resetScore：清空所有列并重置光标

- 复杂度与性能
  - insertNoteAt/backspaceAt：O(N)，其中 N 为列数，适合 DEFAULT_COLUMNS=125 的场景
  - 其他操作：O(1)
