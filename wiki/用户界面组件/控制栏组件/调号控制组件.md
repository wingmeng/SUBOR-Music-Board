...
Sequencer->>Engine : stopAll()（播放中立即丢弃所有声）
Sequencer->>Sequencer : 120ms 防抖后 restartFromCurrentPosition() 从当前列重排
...
#### 播放中调号切换（2026-08-30 改）
序列器采用「立即丢弃 + 防抖重启」的实时调号切换机制：

```mermaid
stateDiagram-v2
[*] --> Playing
Playing --> Discarding : 调号变更(playing)
Discarding --> Debouncing : stopAll() 丢弃所有声
Debouncing --> Rescheduling : 120ms 防抖
Rescheduling --> SchedulingNew : restartFromCurrentPosition()<br/>从当前指示器列 scheduleAllNotes
SchedulingNew --> Playing : 播放继续（指示器与音频同步）
state Discarding {
[*] --> StopAllOscillators
StopAllOscillators --> [*]
}
state Rescheduling {
[*] --> CalculatingCurrentCol
CalculatingCurrentCol --> ResettingWallStart
ResettingWallStart --> [*]
}
```
...
### 实时切换优化
序列器实现了实时调号切换，避免播放中断与叠加混响：
- **立即丢弃**：播放中变更时 stopAll 丢弃所有声，杜绝新旧调音符叠加
- **防抖重启**：120ms 防抖（restartTimer）合并连点，从当前指示器列整体重排
- **同步对齐**：重置 playbackWallStart（约 100ms 与调度基准对齐），保持指示器与音频同步
...
#### 播放中调号切换异常
**症状**：调号切换时出现音频中断、混响叠加或指示器错位
**可能原因**：
- 音频上下文未正确初始化
- 播放中变更未走 requestPlaybackRestart（stopAll + 防抖重启）
- 防抖定时器未在暂停/停止时清除

**解决步骤**：
1. 确保音频上下文在用户交互后初始化
2. 确认 setKeySignature 命中 playing 时调用 requestPlaybackRestart
3. 检查 restartFromCurrentPosition 后 playbackWallStart 与调度基准对齐
