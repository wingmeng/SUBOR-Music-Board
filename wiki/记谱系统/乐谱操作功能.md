...
- 关键操作（均由 useNotation.ts 提供）
  ...
  - undo() / redo()：撤销 / 重做（2026-08-31 新增）

- 撤销/重做机制（2026-08-31 新增）
  - **快照栈**：`undoStack` / `redoStack`，快照 = 修改前完整乐谱深拷贝 + 光标位置，栈上限 100
  - **记录时机**：每个内容修改入口（setNote / clearNote / insertNoteAt / backspaceAt / resetScore / loadScore）在修改前 `pushSnapshot()`；新修改清空重做栈
  - **不进入历史**：moveCursor 纯列扩展、syncColumns 列数对齐
  - **快捷键**：Ctrl/Cmd+Z 撤销，Ctrl/Cmd+Shift+Z 或 Ctrl/Cmd+Y 重做；播放中与对话框打开时禁用
  - **撤销后收尾**（App.vue）：cancelPendingInput 取消待处理输入 → syncColumns 对齐列数 → focusCell 恢复焦点与羽毛笔
  - 详见 history-docs/撤销重做功能实现规范.md
...
复杂度
- resetScore：O(N)
- loadScore：O(M)，M 为新乐谱列数（完整加载不截断）
...
- 最佳实践
  ...
  - 在批量导入前暂停播放，导入后再恢复
  - 合理设置 BPM 与调号；播放中变更时 sequencer 走「stopAll 丢弃 + 120ms 防抖 + 从当前指示器列整体重排」，保证切换平滑
