...
- 乐谱数据结构 Score
  ...
  - 初始为DEFAULT_COLUMNS列（默认125），可随输入推进或导入动态扩展，不再固定上限
...
- 默认列数 DEFAULT_COLUMNS
  - 常量：125
  - 作用：乐谱的初始列数与补齐基线（不足时填充空白列），并非硬上限——超出部分由网格换行生成新行并以滚动条浏览
...
- 有效性与空白列
  - 乐谱列数不固定（初始对齐可视容量，随输入/导入动态扩展），实际有效长度由最后一列存在非空音符决定
...
### DEFAULT_COLUMNS：初始列数与动态扩展机制
- 初始列数
  - DEFAULT_COLUMNS=125，作为初始化基线
  - 初始化 / 导入 / 清空 / 窗口变化时，useNotation.syncColumns 将列数对齐到「不产生滚动条」的可视容量：不足补齐铺满、空白尾部裁掉、含真实内容则保留全部（滚动）

- 动态扩展与加载
  - useNotation.loadScore完整加载导入数据，不截断（保留超出默认列数的部分）
  - setNote/insertNoteAt/moveCursor在列索引超出当前长度时自动追加空白列，输入越过末尾即生成新列/新行
  - 网格全量渲染乐谱列，超出可视区域时由滚动条承接

- 与播放器的协同
  - getEffectiveLength()在播放时动态计算有效长度，避免在空白列上浪费时间
  - setBpm/setKeySignature在播放中变更参数时，走 requestPlaybackRestart()：立即 stopAll 丢弃所有声，120ms 防抖后从当前指示器列整体重排（2026-08-30 改）
...
- 实时变速/变调的丢弃重排
  - setBpm/setKeySignature在播放中变更时，stopAll 立即丢弃所有声，120ms 防抖后从当前指示器列整体重排，杜绝叠加混响并保持音画同步
- 内存占用
  - Score按需扩展（默认125列基线 + 输入/导入动态增长），列数通常为数百量级，渲染与GC开销可控
...
- 导入数据长度
  - loadScore完整加载导入数据不截断，不足DEFAULT_COLUMNS列时用空白列补齐，超出部分由网格滚动浏览
