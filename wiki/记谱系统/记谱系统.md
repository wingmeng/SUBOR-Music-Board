...
## 核心组件
- Score 数据结构：二维数组，每列包含三个声部的记谱字符串，顺序对应 VoiceIndex（0=主旋律，1=和弦律，2=低频）。列数默认 125。
...
- 键盘快捷键：方向键上下左右移动；Backspace 删除前一格并左移填补、末位补空格；Delete 清空当前格且不移位。
...
- 连音线：输入 "-" 表示延音线，延续前一个音符的时值，每格对应一个八分音符；延音线不触发播放。
...
- 列数限制：默认列数为 125，避免超大数组带来的内存与渲染压力。
