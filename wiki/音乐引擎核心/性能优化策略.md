...
- BPM/调号实时调整：setBpm 与 setKeySignature 在播放中仅停止未来音符，避免卡顿
...
- BPM 调整后音符错位
  - 确认 setBpm 流程中是否先调用 stopFrom 再重新调度
  - 检查 playbackWallStart 是否按新 BPM 重新计算
- UI 不同步
...
## 结论
本音乐引擎通过预调度机制与精确的时间控制，实现了低延迟、高同步性的音频播放。活跃振荡器的 Map 追踪与选择性停止策略，确保了在播放中调整 BPM 或调号时的平滑体验。配合合理的内存管理与监控手段，系统在性能与稳定性方面表现优异。建议在实际部署中结合浏览器性能分析工具，持续优化 UI 定时器与音频调度策略。

## 附录
- 优化案例与性能测试结果对比
  - 案例一：BPM 从 90 调整至 120
    - 优化前：播放中卡顿明显，未来音符错位
    - 优化后：仅停止未来音符，当前音符自然结束，无卡顿
  - 案例二：调号从 C 调切换至 D 调
    - 优化前：切换瞬间出现点击声与音高跳跃
    - 优化后：仅停止未来音符，当前音符自然结束，点击声消除
  - 案例三：UI 定时器间隔优化
