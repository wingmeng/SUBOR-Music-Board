...
- 计算列间隔与音符时长：每格时长 = 30 / BPM（BPM 为速度档位）
- 预调度：一次性计算所有剩余音符的绝对起始时间并创建节点
- BPM/调号动态切换（2026-08-30 改）：stopAll 立即丢弃所有声，120ms 防抖后从当前指示器列整体重排，避免混响与错位
...
- DEFAULT_COLUMNS：初始列数 125（乐谱可动态扩展，非硬上限）
...
- 实时变速/变调的丢弃重排：BPM/调号变更时立即 stopAll 丢弃所有声，120ms 防抖后从当前指示器列整体重排，避免多版本叠加混响
...
- 播放卡顿或跳过：确认 BPM/调号动态切换走 requestPlaybackRestart（stopAll + 防抖重启）；检查防抖定时器是否在 pause()/stop() 清除；确认 playbackWallStart 与调度基准对齐
  - 参考路径：[sequencer.ts:55-125](file://src/core/sequencer.ts#L55-L125)
...
- 预调度音符（方波/三角波）
  - [music-engine.ts:90-155](file://src/core/music-engine.ts#L90-L155)
- 动态 BPM 切换（stopAll 丢弃 + 120ms 防抖 + 从当前指示器列整体重排）
  - [sequencer.ts:55-125](file://src/core/sequencer.ts#L55-L125)
- 动态调号切换（stopAll 丢弃 + 120ms 防抖 + 从当前指示器列整体重排）
  - [sequencer.ts:55-125](file://src/core/sequencer.ts#L55-L125)
- 简谱解析与 MIDI 转换
  - [note-map.ts:35-100](file://src/core/note-map.ts#L35-L100)
- **三声部频率与倍率计算**
