...
- BPM 与调号动态切换（2026-08-30 改）：
  - 播放中调整 BPM 或调号时，立即 stopAll 丢弃所有声，120ms 防抖后从当前指示器列整体重排，杜绝多版本叠加混响。
- 三角波时长：
...
- 实时变速/变调的丢弃重排：调整 BPM/调号时立即 stopAll 丢弃所有声，防抖后从当前指示器列整体重排，保持音画同步。
...
- 调号切换异常
  - 确认播放中切换走 requestPlaybackRestart（stopAll + 防抖后从当前列重排），并核对 note-map 映射。
